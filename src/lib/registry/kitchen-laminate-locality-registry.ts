/**
 * Kitchen Material × Locality Registry — Batch 3b (Laminate)
 * URL pattern: /laminate-modular-kitchen-{locality-slug}
 */

export type KitchenLaminateLocalityEntry = {
  urlSlug: string;
  localitySlug: string;
  localityName: string;
  materialSlug: "laminate";
...
const BASE = 117000; // 180000 * 0.65 — laminate is more affordable than acrylic
...
// ─── Raw locality data ────────────────────────────────────────────────────────
type Raw = { slug: string; name: string; city: KitchenLaminateLocalityEntry["citySlug"]; cityName: string; aff: KitchenLaminateLocalityEntry["affluence"]; nearby: string[] };
...
function makeEntry(r: Raw): KitchenLaminateLocalityEntry {
  const { min, max } = price(r.city, r.aff);
  return {
    urlSlug: `laminate-modular-kitchen-${r.slug}`,
    localitySlug: r.slug,
    localityName: r.name,
    materialSlug: "laminate",
    materialName: "Laminate",
...
export const KITCHEN_LAMINATE_LOCALITY_REGISTRY: readonly KitchenLaminateLocalityEntry[] =
  RAW.map(makeEntry);

export function getLaminateLocalityEntry(urlSlug: string): KitchenLaminateLocalityEntry | undefined {
  return KITCHEN_LAMINATE_LOCALITY_REGISTRY.find((e) => e.urlSlug === urlSlug);
}
...
export function buildLaminateLocalityFaqs(e: KitchenLaminateLocalityEntry) {
  const { localityName, cityName, priceMin, priceMax } = e;
  return [
    {
      q: `How much does a laminate modular kitchen cost in ${localityName}?`,
      a: `Laminate modular kitchens in ${localityName} range from ${fmt(priceMin)} to ${fmt(priceMax)} depending on size, layout, and hardware. Laminate offers a durable matte or semi-gloss finish at the most competitive price point, making it an excellent choice for budget-conscious homeowners. FurniRevive provides a free site visit and transparent quote.`,
    },
    {
      q: `Is laminate a good finish for kitchens in ${localityName}?`,
      a: `Absolutely. Laminate is one of the most practical kitchen finishes for ${localityName} homes — it is scratch-resistant, available across a wide design range including wood grains and solids, easy to maintain with a simple wipe-down, and highly budget-friendly. It holds up well to everyday cooking in ${cityName}'s climate.`,
    },
    {
      q: `Do you install laminate modular kitchens in ${localityName}, ${cityName}?`,
      a: `Yes. FurniRevive designs and installs laminate modular kitchens across ${localityName} and the wider ${cityName} region. Our team handles measurement, 3D design, factory manufacturing, delivery, and installation — typically completed within 2–4 weeks.`,
    },
    {
      q: `What warranty do laminate kitchens come with in ${localityName}?`,
      a: `All laminate kitchens installed by FurniRevive in ${localityName} carry a 10-year structural warranty on the carcass, 5-year laminate shutter warranty, and 5-year hardware warranty. We also provide 6 months of free post-installation support.`,
    },
  ] as const;
}
