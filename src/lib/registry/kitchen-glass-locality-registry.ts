/**
 * Kitchen Material × Locality Registry — Glass
 * URL pattern: /glass-modular-kitchen-{locality-slug}
 */

export type KitchenGlassLocalityEntry = {
  urlSlug: string;
  localitySlug: string;
  localityName: string;
  materialSlug: "glass";
...
type Raw = { slug: string; name: string; city: KitchenGlassLocalityEntry["citySlug"]; cityName: string; aff: KitchenGlassLocalityEntry["affluence"]; nearby: string[] };
...
function makeEntry(r: Raw): KitchenGlassLocalityEntry {
  const { min, max } = price(r.city, r.aff);
  return {
    urlSlug: `glass-modular-kitchen-${r.slug}`,
    localitySlug: r.slug,
    localityName: r.name,
    materialSlug: "glass",
    materialName: "Glass",
...
export const KITCHEN_GLASS_LOCALITY_REGISTRY: readonly KitchenGlassLocalityEntry[] =
  RAW.map(makeEntry);

export function getGlassLocalityEntry(urlSlug: string): KitchenGlassLocalityEntry | undefined {
  return KITCHEN_GLASS_LOCALITY_REGISTRY.find((e) => e.urlSlug === urlSlug);
}
...
export function buildGlassLocalityFaqs(e: KitchenGlassLocalityEntry) {
  const { localityName, cityName, priceMin, priceMax } = e;
  return [
    {
      q: `How much does a glass modular kitchen cost in ${localityName}?`,
      a: `Glass modular kitchens in ${localityName} range from ${fmt(priceMin)} to ${fmt(priceMax)} depending on size, layout, and hardware. Glass is a premium finish that delivers a striking transparent or frosted look, elevating the overall aesthetic of any kitchen. FurniRevive provides a free site visit and transparent quote.`,
    },
    {
      q: `Is glass a good finish for kitchens in ${localityName}?`,
      a: `Yes. Glass kitchen shutters are an excellent choice for ${localityName} homes seeking a contemporary, high-end aesthetic. The finish is incredibly easy to wipe clean, resistant to stains and grease marks, and available in back-painted options that allow you to match any colour palette. Frosted glass variants add a soft, diffused elegance, while clear glass creates an open, airy feel — both well-suited to modern homes in ${cityName}.`,
    },
    {
      q: `Do you install glass modular kitchens in ${localityName}, ${cityName}?`,
      a: `Yes. FurniRevive designs and installs glass modular kitchens across ${localityName} and the wider ${cityName} region. Our team handles measurement, 3D design, factory manufacturing, delivery, and installation — typically completed within 2–4 weeks.`,
    },
    {
      q: `What warranty do glass kitchens come with in ${localityName}?`,
      a: `All glass kitchens installed by FurniRevive in ${localityName} carry a 10-year structural warranty on the carcass, 5-year glass shutter warranty, and 5-year hardware warranty. We also provide 6 months of free post-installation support.`,
    },
  ] as const;
}
