...
const LOCALITY = "emaar-palm-drive-gurgaon" as const;
...
if (!auditResult.valid) {
  if (import.meta.env.DEV) {
    console.warn(`[SEO Audit] furniture-repair-emaar-palm-drive-gurgaon — ${auditResult.errors.length} error(s):`, auditResult.errors);
  } else {
    console.error(`[SEO Audit] furniture-repair-emaar-palm-drive-gurgaon — audit failed:`, auditResult.errors);
  }
}
const _renderedPage = renderSeoPage(resolvedPage);
const pageData = buildPageData(SERVICE, LOCALITY, resolvedPage);

export default function FurnitureRepairEmaarPalmDriveGurgaon() {
